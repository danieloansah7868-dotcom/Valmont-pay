...
/**
 * Environment variables that pin this tenant's API secrets.
 *
 * A tenant-specific env var REPLACES the whole key list (see
 * applyEnvOverrides): when SECRET_KEY_1 is set, the database value is never
 * consulted at all. That makes the admin "Rotate keys" button a silent
 * no-op — it writes a new key to the database and nothing changes.
 *
 * Operators must be told that, not left to discover it. These helpers name
 * the variables involved and say whether rotation is actually possible.
 *
 * @returns {{vars: string[], blocksRotation: boolean}}
 */
function envPinnedSecretVars(tenant) {
  const vars = [];
  if (!tenant || !tenant.key) return { vars, blocksRotation: false };

  const prefix = `TENANT__${tenant.key.toUpperCase().replace(/-/g, '_')}__`;
  for (const slot of ['SECRET_KEY_1', 'SECRET_KEY_2']) {
    if (String(process.env[`${prefix}${slot}`] || '').trim()) vars.push(`${prefix}${slot}`);
  }

  // Only pinning slot 1 makes rotation inert: slot 2 still leaves the
  // database in control of secret_key_1, which rotation does change.
  const blocksRotation = vars.includes(`${prefix}SECRET_KEY_1`);
  return { vars, blocksRotation };
}

/**
 * A display name that slugifies onto ANOTHER tenant's key.
 *
 * getTenantByIdentifier() tries `<display-name-with-hyphens>` as a tenant key
 * before it falls back to matching display names, so a merchant called
 * "Valmont Gadget" would capture lookups belonging to the existing
 * `valmont-gadget`. Returns the shadowed key, or null.
 */
function shadowedTenantKey(key, displayName) {
  if (!displayName) return null;
  const slug = normalizeTenantKey(normalizeDisplayName(displayName).replace(/[\s_]+/g, '-'));
  if (!slug || !registry.has(slug)) return null;
  const targetKey = normalizeTenantKey(key);
  return slug === targetKey ? null : slug;
}
...
function validateTenantIdentityForCreate(input) {
  ...
  const shadowed = shadowedTenantKey(key, input && input.display_name);
  if (shadowed) {
    return {
      valid: false,
      reason: `"${input.display_name}" resolves to the existing tenant "${shadowed}" by name. ` +
        'Pick a display name that does not match another tenant\u2019s key.'
    };
  }
  return { valid: true };
}
...
function validateTenantIdentityForUpdate(key, patch) {
  ...
  // Renaming a tenant must not let it capture another tenant's lookups.
  if (requestedName !== undefined) {
    const shadowed = shadowedTenantKey(normalizedKey, requestedName);
    if (shadowed) {
      return {
        valid: false,
        reason: `"${requestedName}" resolves to the existing tenant "${shadowed}" by name. ` +
          'Pick a display name that does not match another tenant\'s key.'
      };
    }
  }
...
/**
 * A credential that may only ever exist on a developer's machine.
 *
 * The literals below are committed to this repository, so anyone who has
 * read the source knows them. That is fine for `npm test` and for a laptop,
 * and indefensible on a deployed gateway: precedence is env > DB > default,
 * so a deployment with no `TENANT__…__SECRET_KEY_1` and no database row
 * would silently authenticate the internet with a public string.
 *
 * In a deployed environment these therefore resolve to empty, which fails
 * closed — the tenant simply has no usable credential until an operator
 * sets one. Local runs are unaffected.
 */
function devOnlyCredential(value) {
...
const DEFAULT_TENANTS = {
  'valmont-electricals': {
    ...
    secret_keys: [
      process.env.TENANT__VALMONT_ELECTRICALS__SECRET_KEY_1
        || devOnlyCredential('vme_secret_dev_key_1'),
      process.env.TENANT__VALMONT_ELECTRICALS__SECRET_KEY_2 || ''
    ].filter(Boolean),
    public_key: process.env.TENANT__VALMONT_ELECTRICALS__PUBLIC_KEY
      || devOnlyCredential('vme_pub_dev_key_1'),
    webhook_signing_secret: process.env.TENANT__VALMONT_ELECTRICALS__WEBHOOK_SIGNING_SECRET
      || process.env.WEBHOOK_SECRET
      || devOnlyCredential('vme_webhook_signing_dev_1'),
    ...
    // Per-tenant receipt targets. Empty means "use the gateway-wide
    // MERCHANT_NOTIFICATION_* settings", which is today's behaviour.
    notification_email: '',
    notification_phone: '',
...
function applyEnvOverrides(tenant) {
  ...
  assign('NOTIFICATION_EMAIL', 'notification_email');
  assign('NOTIFICATION_PHONE', 'notification_phone');
...
function listAllTenants() {
  return Array.from(registry.values()).map(t => ({
    ...
    has_paystack_key: Boolean(t.paystack_secret_key),
    has_webhook_url: Boolean(t.webhook_url),
    keys_pinned_by_env: envPinnedSecretVars(t).vars,
    rotation_blocked_by_env: envPinnedSecretVars(t).blocksRotation,
    notification_email: t.notification_email || '',
    notification_phone: t.notification_phone || '',
    created_at: t.created_at,
    updated_at: t.updated_at
