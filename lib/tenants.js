...
const { isProductionRuntime, rejectInsecureSecret } = require('./insecure-secrets');
...
const DEFAULT_TENANTS = {
  'valmont-electricals': {
    ...
    secret_keys: [
      process.env.TENANT__VALMONT_ELECTRICALS__SECRET_KEY_1 || (isProductionRuntime() ? '' : 'vme_secret_dev_key_1'),
      process.env.TENANT__VALMONT_ELECTRICALS__SECRET_KEY_2 || ''
    ].filter(Boolean),
    public_key: process.env.TENANT__VALMONT_ELECTRICALS__PUBLIC_KEY || (isProductionRuntime() ? '' : 'vme_pub_dev_key_1'),
    webhook_signing_secret: process.env.TENANT__VALMONT_ELECTRICALS__WEBHOOK_SIGNING_SECRET
      || process.env.WEBHOOK_SECRET
      || (isProductionRuntime() ? '' : 'vme_webhook_signing_dev_1'),
  ...
  'valmontweb': {
    ...
    paystack_public_key: process.env.PAYSTACK_PUBLIC_KEY || '',
    secret_keys: [
      process.env.TENANT__VALMONTWEB__SECRET_KEY_1 || (isProductionRuntime() ? '' : 'vmw_secret_dev_key_1'),
      process.env.TENANT__VALMONTWEB__SECRET_KEY_2 || ''
    ].filter(Boolean),
    public_key: process.env.TENANT__VALMONTWEB__PUBLIC_KEY || (isProductionRuntime() ? '' : 'vmw_pub_dev_key_1'),
    webhook_signing_secret: process.env.TENANT__VALMONTWEB__WEBHOOK_SIGNING_SECRET
      || process.env.WEBHOOK_SECRET
      || (isProductionRuntime() ? '' : 'vmw_webhook_signing_dev_1'),
    // Display-only label; never infer payment routing from it.
    environment: 'live',
    status: 'active',
    disabled: false,
...
function getTenantBySecretKey(secretKey) {
  ...
  if (rejectInsecureSecret(normalized)) {
    console.error('[TENANTS] Refusing well-known development secret in production');
    return undefined;
  }
 ...
 * @returns {object}
 */
/** Public checkout branding — never webhook URLs, settlement accounts, or domains. */
function publicTenantBranding(tenant) {
  if (!tenant) return null;
  return {
    key: tenant.key,
...
/**
 * Sanitise a tenant object for admin consumption (no API/Paystack secrets).
 * Webhook URL and settlement account stay here — the public checkout
 * must use publicTenantBranding() instead.
 */
