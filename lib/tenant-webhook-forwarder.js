...
const { rejectInsecureSecret } = require('./insecure-secrets');
const { validatePublicHttpsUrl } = require('./url-safety');
...
function resolveSigningSecret(tenant) {
  ...
  if (tenant.webhook_signing_secret) {
    if (rejectInsecureSecret(tenant.webhook_signing_secret)) {
      console.error(`[WEBHOOK-FWD] ERROR: tenant "${tenant.key}" still has a well-known development signing secret — refusing to sign`);
      return null;
    }
    return tenant.webhook_signing_secret;
  }
  console.error(`[WEBHOOK-FWD] ERROR: tenant "${tenant.key}" has no webhook_signing_secret — refusing to sign webhook`);
  return null;
}
 ...
 */
async function attemptDelivery(tenant, reference, payload, attempt = 0) {
  const webhookUrl = tenant.webhook_url;
  if (!webhookUrl) {
    return { ok: false, statusCode: 0, error: 'No webhook URL configured for tenant' };
  }
  const urlCheck = validatePublicHttpsUrl(webhookUrl, { purpose: 'webhook_url' });
  if (!urlCheck.ok) {
    return { ok: false, statusCode: 0, error: urlCheck.reason };
  }

  const rawBody = JSON.stringify(payload);
  const secret = resolveSigningSecret(tenant);
  if (!secret) {
    console.error(`[WEBHOOK-FWD] ERROR: refusing webhook delivery for "${tenant.key}" — no webhook_signing_secret`);
    return { ok: false, statusCode: 0, error: 'No webhook_signing_secret configured for tenant' };
  }
  ...
  try {
    ...
    // Log the delivery attempt in memory
    logDelivery({
      ...logEntry,
      // map to what in-memory log expects if different
      durationMs: durationMs 
    });

    // Persist to Supabase
    deliveryStore.recordDelivery({
      ...logEntry,
      response_status: response.status,
      response_time_ms: durationMs,
      retry_count: attempt,
      request_body: rawBody
    }).catch(err => console.error('[WEBHOOK-FWD] Persistent log failed:', err.message));
...
function dispatchWebhook(tenant, event, data, gatewayReference, options = {}) {
  ...
  const scheduleNext = () => {
    ...
    const timer = setTimeout(async () => {
      const result = await attemptDelivery(tenant, reference, payload, attempt + 1);
  ...
  return attemptDelivery(tenant, reference, payload, 0).then(result => {
    if (result.ok) {
...
/**
 * Durable retry worker. Reads failed rows from Supabase (survives
 * serverless freeze) and re-dispatches each unique tenant+reference once.
 * Intended to be hit by Vercel Cron every few minutes.
 */
async function retryFailedDeliveries(options = {}) {
  const tenants = options.tenants || require('./tenants');
  const limit = options.limit || 40;
  const { deliveries, ok, reason } = await deliveryStore.fetchFailedDeliveries(limit, options);
  if (!ok) return { retried: 0, skipped: 0, reason: reason || 'lookup-failed' };
  ...
  for (const row of deliveries || []) {
    const key = `${row.tenant_key}::${row.reference}`;
    if (seen.has(key)) { skipped += 1; continue; }
    seen.add(key);

    const tenant = tenants.getTenant(row.tenant_key);
    if (!tenant || !tenant.webhook_url) { skipped += 1; continue; }

    let payload = null;
    if (row.request_body) {
      try { payload = JSON.parse(row.request_body); } catch (_) { payload = null; }
    }
    const eventName = (payload && payload.event) || row.event || 'charge.success';
    const data = (payload && payload.data) || { reference: row.reference };

    const result = await dispatchWebhook(tenant, eventName, data, row.reference, { isRetry: true });
    retried += 1;
    results.push({ reference: row.reference, tenant_key: row.tenant_key, ok: Boolean(result && result.ok) });
  }

  return { retried, skipped, results };
}

module.exports = { resolveSigningSecret, 
  signPayload,
  buildWebhookPayload,
  attemptDelivery,
  dispatchWebhook,
