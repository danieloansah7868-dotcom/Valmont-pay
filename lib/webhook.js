...
const paystackCredentials = require('./paystack-credentials');
...
function handleWebhookEvent(body, signature, rawBody) {
  ...
  // Signature check. Skipped only when no signing secret is configured (local
  // dev), never when one exists — a bad signature is always rejected. The exact
  // raw request bytes are verified, never a re-serialized payload.
  //
  // A tenant with its own Paystack account signs with THAT account's secret
  // key, so the gateway credential alone is no longer sufficient: every
  // configured credential is tried. Without this, a tenant's real payments
  // would be rejected as forgeries and never reach the ledger.
  const secret = getWebhookSecret();
  if (secret) {
    const raw = rawBody || JSON.stringify(payload);
    if (!paystackCredentials.matchesAnyWebhookSecret(raw, signature)) {
      console.warn('[WEBHOOK] Rejected event with an invalid signature.');
      return { statusCode: 401, body: { status: false, message: 'Invalid signature' } };
    }
  } else {
    console.warn(
      '[WEBHOOK] Neither WEBHOOK_SECRET nor PAYSTACK_SECRET_KEY is set — skipping signature verification.'
    );
  }
