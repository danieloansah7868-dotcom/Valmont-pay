 ...
 */
async function persistPaymentLink({ accessCode, payment, ttlMs, client: injectedClient }) {
  ...
  if (!isSupabaseConfigured() && !injectedClient) {
  ...
  const client = resolveClient(injectedClient);
  if (!client) {
