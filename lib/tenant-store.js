...
async function createTenant(input, options = {}) {
  ...
  if (record.webhook_url) {
    const check = validatePublicHttpsUrl(record.webhook_url, { purpose: 'webhook_url' });
    if (!check.ok) return fail(check.reason);
    record.webhook_url = check.url;
  }
...
async function updateTenant(key, patch, options = {}) {
  ...
  if (updates.webhook_url) {
    const check = validatePublicHttpsUrl(updates.webhook_url, { purpose: 'webhook_url' });
    if (!check.ok) return fail(check.reason);
...
async function rotateSecrets(key, options = {}) {
  ...
  try {
    ...
    const newSigning = crypto.randomBytes(32).toString('hex');
    const { data, error } = await client
      .from('tenants')
      .update({
        secret_key_1: newKey,
        secret_key_2: existing.secret_key_1 || null,
        webhook_signing_secret: newSigning
      })
      .eq('key', key.toLowerCase())
      .select('*').single();
    if (error) return fail(describeError(error));
