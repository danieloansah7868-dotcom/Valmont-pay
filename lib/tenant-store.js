 ...
 *
 * This is the SOURCE OF TRUTH for tenants configured via the Tenants admin
 * UI (/tenants.html). It works alongside lib/tenants.js:
 *
 *   1. lib/tenants.js holds the in-memory registry (env-var defaults,
 *      built-in seed tenants, etc.).
 *   2. On server boot (and on demand) this module loads every tenant from
 *      the `tenants` table in Supabase and merges them OVER the in-memory
 *      registry. DB tenants always win.
 *   3. CRUD mutations in this module persist to Supabase AND update the
 *      in-memory registry atomically, so the next webhook delivery sees
 *      the new webhook URL immediately without a redeploy.
 *
