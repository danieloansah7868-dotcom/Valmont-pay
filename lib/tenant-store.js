...
async function createTenant(input, options = {}) {
  ...
  try {
    // REFUSE TO CLOBBER AN EXISTING MERCHANT.
    //
    // The upsert below is ON CONFLICT (key) DO UPDATE, and this record always
    // carries a freshly generated secret_key_1. Re-running a create for a key
    // that already exists would therefore overwrite the live tenant AND rotate
    // its API secret out from under a working integration — with a 201
    // "Tenant created" response and no warning. A typo in the slug field
    // should never be able to take a merchant offline.
    ...
    if (existing && existing.error === null && existing.data) {
      return fail(
        `A tenant with the key "${key}" already exists. Edit it instead of creating it again — ` +
          're-creating would overwrite the record and issue a new API secret, breaking any live integration.'
      );
    }
