...
const CANONICAL_WEBHOOK_URL = 'https://valmont-pay.vercel.app/api/webhook';

/** Paystack signs webhooks with the SECRET KEY; WEBHOOK_SECRET is an override. */
...
function detectPublicOrigin(req) {
  ...
  const proto = headers['x-forwarded-proto'] || (req && req.protocol) || 'https';
  const host =
...
function configurationChecks(req) {
  ...
  checks.push({
    ...
    detail: effectiveSecret
      ? `Signing with ${process.env.WEBHOOK_SECRET ? 'WEBHOOK_SECRET' : 'PAYSTACK_SECRET_KEY'}.`
      : 'Neither WEBHOOK_SECRET nor PAYSTACK_SECRET_KEY is set, so every signature check fails.',
    fix: effectiveSecret
      ? null
      : 'Add PAYSTACK_SECRET_KEY to your Vercel Project → Settings → Environment Variables, then redeploy.'
  });

  // 2. Checklist item: "Is WEBHOOK_SECRET the same as PAYSTACK_SECRET_KEY?"
  const secretsMatch = Boolean(webhookSecret) && webhookSecret === paystackKey;
  checks.push({
    id: 'webhook-secret-matches-paystack-key',
    label: 'WEBHOOK_SECRET matches PAYSTACK_SECRET_KEY',
    status: !webhookSecret ? 'pass' : secretsMatch ? 'pass' : 'fail',
    detail: !webhookSecret
      ? 'WEBHOOK_SECRET is not set, so PAYSTACK_SECRET_KEY is used directly. This is the recommended setup.'
      : secretsMatch
        ? 'WEBHOOK_SECRET is set and identical to PAYSTACK_SECRET_KEY (fingerprints match).'
        : 'WEBHOOK_SECRET is set but DIFFERENT from PAYSTACK_SECRET_KEY. Paystack signs with the secret key, ' +
          'so every incoming signature will be rejected with HTTP 400.',
    fix:
      !webhookSecret || secretsMatch
        ? null
        : 'Delete WEBHOOK_SECRET in Vercel (preferred), or set it to the exact same value as PAYSTACK_SECRET_KEY, then redeploy.',
    fingerprints: {
  ...
  const urlMatches = Boolean(liveWebhookUrl) && liveWebhookUrl === CANONICAL_WEBHOOK_URL;
  checks.push({
    id: 'webhook-url',
    label: `Webhook URL is exactly ${CANONICAL_WEBHOOK_URL}`,
    ...
    detail: !liveWebhookUrl
      ...
      : urlMatches
        ...
        : `This request arrived at ${liveWebhookUrl}. That is a valid endpoint, but it is not the canonical ` +
          `${CANONICAL_WEBHOOK_URL}. Paystack only calls the ONE url saved in your dashboard.`,
    fix:
      urlMatches
        ? null
        : `In Paystack → Settings → API Keys & Webhooks, set the ${mode === 'live' ? 'Live' : 'Test'} Webhook URL to ` +
          `${CANONICAL_WEBHOOK_URL} — no trailing slash, no /api/webhook/, https not http.`,
    observed: liveWebhookUrl,
    expected: CANONICAL_WEBHOOK_URL
  });
